...
//(5) s1과 s2의 학번 중 앞선 학번(고학년) 출력
		if (s1.getStdtIDnum() < s2.getStdtIDnum())
			System.out.println(s1.getName() + "은 고학년입니다");
		else
			System.out.println(s2.getName() + "는 고학년입니다");
//(6) s1과 s2의 이름 중 사전적으로(ㄱㄴㄷ..) 앞선 이름 출력
		// compareTo compareToIgnoreCase
		if (((s1.getName()).compareToIgnoreCase(s2.getName())) <= 0)
			System.out.println(s1.getName() + "이 이름이 앞섬");
		else
			System.out.println(s2.getName() + "가 이름이 앞섬");
//(7) s1과 s2의 성적 중 높은 성적 출력
		if (s1.getScore() > s2.getScore())
			System.out.println(s1.getName() + "이 더 성적 높아");
		else
			System.out.println(s2.getName() + "가 더 성적 높아");

//(8) 기준성적을 1.5로 s2의 합격여부를 조회하여 합격/불합격 출력
		if (s1.pNp(s1.getScore()) == true)
			System.out.println(s1.getName() + "합격입니다");
		else
			System.out.println(s1.getName() + "불합격입니다");
//(9) 기준성적을 1.5로 s2의 합격여부를 조회하여 합격/불합격 출력
		if (s2.pNp(s2.getScore()) == true)
			System.out.println(s2.getName() + "합격입니다");
		else
			System.out.println(s2.getName() + "불합격입니다");
